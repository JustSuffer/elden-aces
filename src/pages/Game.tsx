...
const Game = () => {
  ...
  // Initialize game state only when deck is selected
  const gameHook = useGameState(
    selectedDeck && opponentClass 
      ? { playerDeck: selectedDeck, opponentClass }
      : undefined
  );
  ...
  // Show deck selection screen if game hasn't started
  if (!gameStarted) {
  ...
  const handleRollDice = () => {
    // Fateweaver can only roll from round 3+
